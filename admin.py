...
def update_pscore():
    if request.method == "POST":
        ...
        if str(yards) and member_id:
            check_flag = dbOp.update_sql("PLAYER_SCORE", f"yards = {yards}",
                                         f"pscore_id = {member_id} and p_year = {year}")
            if check_flag == 0:
                error_list += "[yards] "

        if str(touchdowns) and member_id:
            check_flag = dbOp.update_sql("PLAYER_SCORE", f"touchdowns = {touchdowns}",
                                         f"pscore_id = {member_id} and p_year = {year}")
            if check_flag == 0:
                error_list += "[touchdowns] "

        if str(total_sacks) and member_id:
            check_flag = dbOp.update_sql("PLAYER_SCORE", f"total_sacks = {total_sacks}",
                                         f"pscore_id = {member_id} and p_year = {year}")
            if check_flag == 0:
                error_list += "[total sacks] "

        if str(total_fumbles) and member_id:
            check_flag = dbOp.update_sql("PLAYER_SCORE", f"total_fumbles = {total_fumbles}",
                                         f"pscore_id = {member_id} and p_year = {year}")
            if check_flag == 0:
                error_list += "[total fumbles] "

        if str(number_games_played) and member_id:
            check_flag = dbOp.update_sql("PLAYER_SCORE", f"number_games_played = {number_games_played}",
                                         f"pscore_id = {member_id} and p_year = {year}")
            if check_flag == 0:
                error_list += "[number of games played] "

        if member_id == 0:
            flash(f'Player Information: Incorrect [{name}] or [{year}]!', 'danger')
            logging.info(f'% Player information: Incorrect [{name}] or [{year}]!', name, year)
        elif not error_list:
            flash(f'Player Information for [{name}], [{year}]: Updated!', 'success')
            logging.info(f'% Player information: [{name}] or [{year}]: Updated!', name, year)
        else:
            flash(f'Player Information for [{name}], [{year}]: {error_list} have not been updated!', 'danger')
            logging.info(f'% Player information: [{name}] or [{year}]: have not been updated!', name, year)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def update_cscore():
    if request.method == "POST":
        ...
        if str(years_coached) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"years_coached = {years_coached}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[Years Coached] "

        if str(total_games_coached) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"total_games_coached = {total_games_coached}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[Total Games Coached] "

        if str(sb_champ_won) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"sb_champ_won = {sb_champ_won}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[Super Bowl Championships Won] "

        if str(league_champ_won) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"league_champ_won = {league_champ_won}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[League Championships Won] "

        if str(conference_champ_won) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"conference_champ_won = {conference_champ_won}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[Conference Championships Won] "

        if str(division_champ_won) and member_id:
            check_flag = dbOp.update_sql("COACH_SCORE", f"division_champ_won = {division_champ_won}",
                                         f"cscore_id = {member_id} and c_year = {year}")
            if check_flag == 0:
                error_list += "[Division Championships Won] "

        if member_id == 0:
            flash(f'Coach Information for: Incorrect [{name}] or [{year}]!', 'danger')
            logging.info(f'%s Incorrect coach information [{name}] or [{year}] entered', name, year)
        elif not error_list:
            flash(f'Coach Information for [{name}], [{year}]: Updated!', 'success')
            logging.info(f'%s Coach information [{name}], [{year}] updated', name, year)
        else:
            flash(f'Coach Information for [{name}], [{year}]: {error_list} have not been updated!', 'danger')
            logging.info(f'%s Coach information [{name}], [{year}] not updated', name, year)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def update_tscore():
    if request.method == "POST":
        ...
        if str(total_field_goals) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"total_field_goals = {total_field_goals}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Total Field Goals]] "

        if str(total_touchdowns) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"total_touchdowns = {total_touchdowns}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Total Touchdowns] "

        if str(total_passing_yards) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"total_passing_yards = {total_passing_yards}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Total Passing Yards] "

        if str(avg_time_possession) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"avg_time_possession = {avg_time_possession}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Average Time Possession] "

        if str(total_rushing_yards) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"total_rushing_yards = {total_rushing_yards}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Total Rushing Yards] "

        if str(total_offensive_yards) and member_id:
            check_flag = dbOp.update_sql("TEAM_SCORE", f"total_offensive_yards = {total_offensive_yards}",
                                         f"t_name = \'{name}\' and year = {year}")
            if check_flag == 0:
                error_list += "[Total Offensive Yards] "

        if member_id == 0:
            flash(f'Team Information: Incorrect [{name}] or [{year}]!', 'danger')
            logging.info(f'%s Incorrect team information [{name}] or [{year}]', name, year)
        elif not error_list:
            flash(f'Team Information for [{name}], [{year}]: Updated!', 'success')
            logging.info(f'%s Team information [{name}], [{year}] updated', name, year)
        else:
            flash(f'Team Information for [{name}], [{year}]: {error_list} have not been updated!', 'danger')
            logging.info(f'%s Team information [{name}], [{year}] not updated', name, year)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
    ...
    if request.method == "POST":
        ...
        if name_flag == 1 and draft_add is not None and check_player is not None or check_coach is not None:  # and team_flag == 1:
            if not minit and not te_name:
                ...
                check_flag = dbOp.insert_sql("TEAM_MEMBER",
                                             f"{rand_id}, \'{fname}\', \'{minit}\', \'{lname}\', \'{te_name}\'")
                if check_flag:
                    dbOp.read_sql_raw(f"UPDATE TEAM SET roster = roster + 1 WHERE team_name = \'{te_name}\'")
            ...
            if minit:
                team_name = dbOp.read_sql_raw(
                    f"SELECT te_name from TEAM_MEMBER WHERE fname=\'{fname}\' and minit=\'{minit}\' and lname=\'{lname}\'")
            else:
                team_name = dbOp.read_sql_raw(
                    f"SELECT te_name from TEAM_MEMBER WHERE fname=\'{fname}\' and minit is null and lname=\'{lname}\'")

            if team_name.empty:
                check_delete = 0
            elif team_name['te_name'].iloc[0] is None:
                if minit:
                    check_delete = dbOp.delete_sql("TEAM_MEMBER",
                                                   f"fname=\'{fname}\' and minit=\'{minit}\' and lname =\'{lname}\'")
                else:
                    check_delete = dbOp.delete_sql("TEAM_MEMBER",
                                                   f"fname=\'{fname}\' and minit is null and lname =\'{lname}\'")

        if name_flag == 0:  # or team_flag == 0:
            flash(f'Drafted Information: Draft [{fname}] or [{lname}] missing!', 'danger')
            logging.info(f'%s Draft information [{fname} or [{lname} missing!', fname, lname)
            ...
            if not inserted_flag and not check_flag:
                ...
                logging.info(f'%s Draft information [{fname} or [{lname} invalid', fname, lname)
            else:
                flash(f'Drafted Information for [{fname}], [{lname}]: Draft added!', 'success')
                logging.info(f'%s Draft information [{fname} or [{lname} added!', fname, lname)
        elif draft_remove is not None:
            if not check_delete:
                flash(f'Drafted Information for [{fname}], [{lname}]: Remove Invalid!', 'danger')
                logging.info(f'%s Draft information [{fname} or [{lname} remove invalid!', fname, lname)
            else:
                flash(f'Drafted Information for [{fname}], [{lname}]: Removed!', 'success')
                logging.info(f'%s Draft information [{fname} or [{lname} removed!', fname, lname)
...
def add_player():
    if request.method == "POST":
        ...
        if update_minit:
            df_exists = pd.Series(
                [str(i) + str(j) + str(k) for i, j, k in df_items[['fname', 'minit', 'lname']].values])
        ...
        df_items = dbOp.read_sql_raw(
            f"SELECT * FROM TEAM as T join TEAM_MEMBER as P on T.team_name = P.te_name WHERE {update_op}")
        previous_team = ''
        ...
        if check_flag:  # or team_flag == 0:
            ...
            logging.info(f'%s Player [{new_player}] was added to team [{update_pteam}]', new_player, update_pteam)
        else:
            flash(f'Player [{new_player}] cannot be added to team [{update_pteam}]!', 'danger')
            logging.info(f'%s Player [{new_player}] cannot be added to team [{update_pteam}]', new_player, update_pteam)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def remove_player():
    if request.method == "POST":
        ...
        if check_flag:  # or team_flag == 0:
            ...
            logging.info(f'%s Player [{new_player}] removed from team [{update_pteam}]', new_player, update_pteam)
        else:
            flash(f'Player [{new_player}]  was not removed from team [{update_pteam}]!', 'danger')
            logging.info(f'%s Player [{new_player}] was not removed from team [{update_pteam}]', new_player, update_pteam)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def add_coach():
    if request.method == "POST":
        ...
        if check_flag:  # or team_flag == 0:
            ...
            logging.info(f'%s Coach [{new_coach}] was added to team [{update_cteam}]', new_coach, update_cteam)
        else:
            flash(f'Coach [{new_coach}] cannot be added to team [{update_cteam}]!', 'danger')
            logging.info(f'%s Coach [{new_coach}] cannot be added to team [{update_cteam}]', new_coach, update_cteam)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def remove_coach():
    if request.method == "POST":
        ...
        if check_flag:  # or team_flag == 0:
            ...
            logging.info(f'%s Coach [{new_coach}] was removed from team [{update_cteam}]', new_coach, update_cteam)
        else:
            flash(f'Coach [{new_coach}]  was not removed from team [{update_cteam}]!', 'danger')
            logging.info(f'%s Coach [{new_coach}] was not removed from team [{update_cteam}]', new_coach, update_cteam)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def add_match():
    if request.method == "POST":
        ...
        identical_check = 1
        df_exists = pd.Series(
            [str(i) + str(j) + str(k) for i, j, k in df_id[['Mte_name1', 'Mte_name2', 'date']].values])
        ...
        if check_flag:  # or team_flag == 0:
            ...
            logging.info(f'%s match between [{t1}] and [{t2}] was added', t1, t2)
        elif identical_check:
            flash(f'Match between [{t1}] and [{t2}] already exists for [{date}]!', 'danger')
            logging.info(f'%s match between [{t1}] and [{t2}] already exists', t1, t2)
        else:
            flash(f'Match between [{t1}] and [{t2}] not added for [{date}]!', 'danger')
            logging.info(f'%s match between [{t1}] and [{t2}] was not added', t1, t2)
...
def update_match():
    if request.method == "POST":
        ...
        if not identical_check:
            flash(f'Match information between [{update_name1}] and [{update_name2}] on [{update_date}] does not exist!',
                  'danger')
            logging.info(f'%s match information between [{update_name1}] and [{update_name2}] check', update_name1, update_name2)
        elif not update_list:
            flash(f'Match information between [{update_name1}] and [{update_name2}] on [{update_date}]: Not updated!',
                  'success')
            logging.info(f'%s match information between [{update_name1}] and [{update_name2}] not updated', update_name1,
                         update_name2)
        else:
            flash(
                f'Match Information between [{update_name1}] and [{update_name2}] on [{update_date}]: {update_list} have been updated!',
                'success')
            logging.info(f'%s match information between [{update_name1}] and [{update_name2}] updated', update_name1,
                         update_name2)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
...
def delete_match():
    if request.method == "POST":
        ...
        if not identical_check:
            flash(f'Match information between [{delete_name1}] and [{delete_name2}] on [{delete_date}] does not exist!',
                  'danger')
            logging.info(f'%s match information between [{delete_name1}] and [{delete_name2}] check', delete_name1,
                         delete_name2)
        elif not check_flag:
            flash(f'Match information between [{delete_name1}] and [{delete_name2}] on [{delete_date}]: Not deleted!',
                  'danger')
            logging.info(f'%s match information between [{delete_name1}] and [{delete_name2}] not deleted', delete_name1,
                         delete_name2)
        else:
            flash(f'Match information between [{delete_name1}] and [{delete_name2}]on [{delete_date}]: Deleted!',
                  'success')
            logging.info(f'%s match information between [{delete_name1}] and [{delete_name2}] deleted', delete_name1,
                         delete_name2)
        return render_template("homeAdmin.html")
    else:
        return render_template("homeAdmin.html")
    ...
    if request.method == "POST":
        ...
        if not df_u.empty:
            if add_user == df_u['admin_ver_name'].iloc[0]:
                ...
                check_flag = dbOp.update_sql("FAN", f" admin_ver_name = \'{add_user}\'",
                                             f"username = \'{add_username}\'")
        else:
            exist_flag = 0
